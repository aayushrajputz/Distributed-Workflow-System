...
const exportTasks = async (req, res) => {
  try {
    const { format = 'csv', startDate, endDate, status, priority, assignee } = req.query;
...
const exportAnalytics = async (req, res) => {
  try {
    const { format = 'csv', startDate, endDate, endpoint, statusCode } = req.query;
    const userId = req.user._id;
    ...
    if (format === 'csv') {
      ...
      res.json({
        exportedAt: new Date().toISOString(),
        totalRecords: exportData.length,
        filters: { startDate, endDate, endpoint, statusCode },
        data: exportData
      });
    } else {
      return res.status(400).json({
...
const exportWorkflowExecutions = async (req, res) => {
  try {
    const { format = 'csv', startDate, endDate, status, templateId } = req.query;
    const userId = req.user._id;
    ...
    if (format === 'csv') {
      ...
      res.json({
        exportedAt: new Date().toISOString(),
        totalRecords: exportData.length,
        filters: { startDate, endDate, status, templateId },
        data: exportData
      });
    } else {
      return res.status(400).json({
